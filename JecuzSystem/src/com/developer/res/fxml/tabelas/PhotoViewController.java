...
public class PhotoViewController implements Initializable {
    ...
    @FXML
    private TableColumn tc_foto;
    @FXML
    private TableColumn tc_entidade;

    private int QUANTIDADE_PAGINA = 6;
    ...
    public final class ItemLista {
        ...
        private PhotoViewController myInstance;
        private Photo entity;

        public ItemLista(int id, int index, ImageView image, String entidade, HBox myBox,
                JFXButton btnEliminar, JFXButton btnEditar,
                PhotoViewController myInstance, Photo entity) {
            this.id = id;
            this.index = index;
            ...
            this.image.setImage(new Image("file:" + Util.configs.getDirector() + "\\" + entity.getPath(), 30, 30, true, true));

            //System.err.println("Existe ? = s" + new File(Util.configs.getDirector() + "\\" + entity.getPath()).exists());
        ...
        public PhotoViewController getMyInstance() {
            return myInstance;
        }
    ...
    private ObservableList<ItemLista> listar(int quantidade, int pagina) {
        ...
        String queryComandss = "SELECT p FROM Photo p";

        Query querys = Util.enti.createQuery(queryComandss, Photo.class).setHint(QueryHints.REFRESH, true)
        ...
        for (Photo obj : lista) {
            ...
            if (!new File(Util.configs.getDirector() + "\\" + obj.getPath()).exists()) {
                Photo ps = Util.enti.merge(obj);
                Util.enti.remove(ps);
                ...
                olis.add(new ItemLista(obj.getId(),
                        i,
                        new ImageView(),
                        obj.getEntityId() != null ? obj.getEntityId().getName()
                        : obj.getProductId() != null ? obj.getProductId().toString() : "Não Está associado...",
                        new HBox(5),
