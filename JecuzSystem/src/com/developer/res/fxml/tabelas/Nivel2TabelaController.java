 ...
 * @author hdglo
 */
public class Nivel2TabelaController implements Initializable {
    ...
    public final class ItemLista {
        ...
        private Nivel2TabelaController myInstance;
        ...
        public ItemLista(int id, int index, String desigancao, String categoria, HBox myBox,
                ...
                Nivel2TabelaController myInstance, Nivel2 entity) {
            this.id = id;
            this.index = index;
        ...
        public Nivel2TabelaController getMyInstance() {
            return myInstance;
        }

        public Nivel2 getEntity() {
            return entity;
        }
    ...
    private ObservableList<ItemLista> listar(int quantidade, int pagina) {
        ...
        String queryComandss = "SELECT p FROM Nivel2 p";

        Query querys = Util.enti.createQuery(queryComandss, Nivel2.class).setHint(QueryHints.REFRESH, true)
                .setMaxResults(quantidade).setFirstResult((pagina * quantidade));

        //querys.setParameter("state", 0);
        List<Nivel2> lista = querys.getResultList();
        Util.enti.getTransaction().commit();
        Util.emf.close();

        int i = 0;
        ObservableList olis = FXCollections.observableArrayList();
        for (Nivel2 obj : lista) {
            ...
            olis.add(new ItemLista(obj.getId(),
                    i,
                    obj.getDesignation(),
                    obj.getNivel1Id().getDesignation(),
                     new HBox(5),
