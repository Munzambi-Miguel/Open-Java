...
import com.developer.res.fxml.produtos.FormeProdutoController;
 ...
 * @author hdglo
 */
public class Nivel1TabelaController implements Initializable {
    ...
    public final class ItemLista {
        ...
        private Nivel1TabelaController myInstance;
        ...
        public ItemLista(int id, int index, String desigancao, HBox myBox, 
                ...
                Nivel1TabelaController myInstance, Nivel1 entity) {
            this.id = id;
            this.index = index;
        ...
        public Nivel1TabelaController getMyInstance() {
            return myInstance;
        }

        public Nivel1 getEntity() {
            return entity;
        }
    ...
    private ObservableList<ItemLista> listar(int quantidade, int pagina) {
        ...
        String queryComandss = "SELECT p FROM Nivel1 p";

        Query querys = Util.enti.createQuery(queryComandss, Nivel1.class).setHint(QueryHints.REFRESH, true)
                .setMaxResults(quantidade).setFirstResult((pagina * quantidade));

        //querys.setParameter("state", 0);

        List<Nivel1> lista = querys.getResultList();
        Util.enti.getTransaction().commit();
        Util.emf.close();

        int i = 0;
        ObservableList olis = FXCollections.observableArrayList();
        for (Nivel1 obj : lista) {
